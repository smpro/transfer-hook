...
def makeViewsLive():
  ...
  q_view_status_streams="""
    ...
    WHERE
      FILE_TRANSFER_STATUS.RUNNUMBER >= (
        SELECT MIN(RUNNUMBER) FROM (SELECT RUNNUMBER FROM CMS_STOMGR.RUNS WHERE RUNNUMBER<999999999 ORDER BY RUNNUMBER DESC) WHERE ROWNUM<=5
      ) AND
  ...
  q_view_status_runs="""
    ...
    FROM ( 
      ...
      FROM CMS_STOMGR.FILE_TRANSFER_STATUS WHERE 
        RUNNUMBER >= (
          SELECT MIN(RUNNUMBER) FROM (SELECT RUNNUMBER FROM CMS_STOMGR.RUNS WHERE RUNNUMBER<999999999 ORDER BY RUNNUMBER DESC) WHERE ROWNUM<=1000
        ) AND
        RUNNUMBER<999999999
      GROUP BY RUNNUMBER
    ) FILE_STATUS_RUNS
  ...
  q_view_run_file_quality="""
    ...
    WHERE
      RUNNUMBER >= (SELECT MIN(RUNNUMBER) FROM (SELECT * FROM CMS_STOMGR.RUNS WHERE RUNNUMBER<999999999 ORDER BY RUNNUMBER DESC) WHERE ROWNUM<=1000) AND
