...
def makeDummyViews():
  ...
  q_mview_stream_file_quality="""
    ...
    AS SELECT
      ...
      SUM(NVL(EVENTS_LOST_OVERSIZED,0)) AS TOTAL_EVENTS_LOST_OVERSIZED, 
      SUM(NVL(FILE_SIZE,0))             AS TOTAL_SIZE                 , 
      COUNT(NVL(EVENTS_BUILT,0))          AS TOTAL_EVENTS_BUILT         , 
      COUNT(NVL(EVENTS_LOST,0))           AS TOTAL_EVENTS_LOST          , 
      COUNT(NVL(EVENTS_LOST_CHECKSUM,0))  AS TOTAL_EVENTS_LOST_CHECKSUM , 
      COUNT(NVL(EVENTS_LOST_CMSSW,0))     AS TOTAL_EVENTS_LOST_CMSSW    , 
      COUNT(NVL(EVENTS_LOST_CRASH,0))     AS TOTAL_EVENTS_LOST_CRASH    , 
      COUNT(NVL(EVENTS_LOST_OVERSIZED,0)) AS TOTAL_EVENTS_LOST_OVERSIZED, 
      COUNT(NVL(FILE_SIZE,0))             AS TOTAL_SIZE 
    FROM CMS_STOMGR.FILE_QUALITY_CONTROL WHERE RUNNUMBER<999999999 AND EVENTS_BUILT>=0 
    GROUP BY RUNNUMBER, STREAM
...
def makeViewsLive():
  ...
  q_view_status_runs="""
    ...
    SELECT
      STREAMS.RUNNUMBER,
      STREAMS.FILES_INJECTED_RUN,
      STREAMS.FILES_TRANSFERRED_RUN,
      STREAMS.FILES_CHECKED_RUN,
      STREAMS.FILES_REPACKED_RUN,
      STREAMS.FILES_INJECTED_RUN + STREAMS.FILES_TRANSFERRED_RUN + STREAMS.FILES_CHECKED_RUN + STREAMS.FILES_REPACKED_RUN AS FILES_TOTAL_RUN,
      STREAMS.FILES_DELETED_RUN,
      STREAMS.FILES_CORRUPTED_RUN,
      STREAMS.LAST_INJECTION_TIME_RUN,
      RSP_CMSSW_VERSION.STRING_VALUE AS CMSSW_VERSION,
      RSP_HLT_KEY.STRING_VALUE AS HLT_KEY,
      THE_RUNS.SETUPLABEL AS LABEL
    FROM 
    (  
      SELECT
        CMS_STOMGR.MVIEW_STATUS_STREAMS.RUNNUMBER              AS RUNNUMBER,
        SUM(CMS_STOMGR.MVIEW_STATUS_STREAMS.FILES_INJECTED)    AS FILES_INJECTED_RUN,
        SUM(CMS_STOMGR.MVIEW_STATUS_STREAMS.FILES_TRANSFERRED) AS FILES_TRANSFERRED_RUN,
        SUM(CMS_STOMGR.MVIEW_STATUS_STREAMS.FILES_CHECKED)     AS FILES_CHECKED_RUN,
        SUM(CMS_STOMGR.MVIEW_STATUS_STREAMS.FILES_REPACKED)    AS FILES_REPACKED_RUN,
        SUM(CMS_STOMGR.MVIEW_STATUS_STREAMS.FILES_REPACKED)    AS FILES_TOTAL_RUN,
        SUM(CMS_STOMGR.MVIEW_STATUS_STREAMS.FILES_DELETED)     AS FILES_DELETED_RUN,
        SUM(CMS_STOMGR.MVIEW_STATUS_STREAMS.FILES_CORRUPTED)   AS FILES_CORRUPTED_RUN,
        MAX(LAST_INJECTION_TIME)                         AS LAST_INJECTION_TIME_RUN
      FROM CMS_STOMGR.MVIEW_STATUS_STREAMS
      GROUP BY RUNNUMBER
      ORDER BY LAST_INJECTION_TIME_RUN DESC
    ...
    INNER JOIN CMS_RUNINFO.RUNSESSION_PARAMETER RSP_CMSSW_VERSION
      ON RSP_CMSSW_VERSION.RUNNUMBER = STREAMS.RUNNUMBER AND RSP_CMSSW_VERSION.NAME=''CMS.DAQ:CMSSW_VERSION''
    INNER JOIN CMS_RUNINFO.RUNSESSION_PARAMETER RSP_HLT_KEY
      ON RSP_HLT_KEY.RUNNUMBER = STREAMS.RUNNUMBER AND RSP_HLT_KEY.NAME=''CMS.DAQ:HLT_KEY''
    INNER JOIN CMS_STOMGR.RUNS THE_RUNS
      ON THE_RUNS.RUNNUMBER = STREAMS.RUNNUMBER';
    EXECUTE IMMEDIATE 'GRANT SELECT ON STATUS_RUNS TO PUBLIC';
    END;
  """
  q_mview_run_file_quality="""
...
def dropViews():
    ...
    databaseAgent.runQuery('CMS_STOMGR', "DROP MATERIALIZED VIEW LOG ON FILE_TRANSFER_STATUS", False)
    databaseAgent.runQuery('CMS_STOMGR', "DROP MATERIALIZED VIEW RUN_FILE_QUALITY", False)
    databaseAgent.runQuery('CMS_STOMGR', "DROP MATERIALIZED VIEW STREAM_FILE_QUALITY", False)
    databaseAgent.runQuery('CMS_STOMGR', "DROP MATERIALIZED VIEW LOG ON FILE_QUALITY_CONTROL", False)
    databaseAgent.cxn_db['CMS_STOMGR'].commit()
# Don't run this in the production environment, ever.
def dropTestTables():
