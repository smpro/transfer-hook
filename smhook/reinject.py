...
full_run_list = sorted(glob.glob(os.path.join('/store/lustre/transfer/', 'run*')), reverse=True)
...
for nf in range(1, min(len(full_run_list),50)):
    ...
    path = "/store/lustre/transfer/run"+number+"/"
    ...
    ## if they are not equal, then continue
        ...
        print 'mv '+dat_file+' /store/lustre/mergeMacro/run'+str(number)+'/stream'+streamName+'/data/'+dat_file_strip
        print 'mv '+jsn_file+' /store/lustre/mergeMacro/run'+str(number)+'/stream'+streamName+'/jsns/'+jsn_file_strip
